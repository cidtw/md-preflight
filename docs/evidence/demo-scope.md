...
### 시연 UI 경로 (매장 선택) — 2026-07-20 갱신

| 경로 | 용도 |
|------|------|
| **세솔로 25 앵커 전수조사** | 시연 본체 · 앵커 `경기도 고양시 덕양구 세솔로 25` · Kakao 전수조사 |
| 편의점 | 반경 **1km** (CS2) |
| 슈퍼 / SSM | 반경 **3km** (키워드 전수) |
| 대형마트·할인점 | 반경 **10km** (MT1 + 필터) |
| API | `GET /api/demo/survey-anchor` · `GET /api/demo/verified-stores` |
| **탐색·더미** | 위저드 자유 입력 · 하단 탐색 프리셋 |

각 매장: 도로명 주소 + 추론 상권(`trade_area`) · 접근성 · 주변 POI 기반 FTI proxy → evaluate 파라미터.  
POS 실측 수요는 아님.
