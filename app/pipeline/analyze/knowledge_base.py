...
def match_knowledge(
    *,
    ...
    # Logistics delay from supply difficulty + accessibility + dong hash residue.
    ...
    # Safety Z: volatility axis + product class + light dong residue.
    vol_norm = scores.demand_volatility / 5.0
    z = 1.15 + vol_norm * 0.85 + _product_volatility_boost(product_name) + unit * 0.25
    safety_z = round(z, 2)
    ...
    # Stockout risk % driven by volatility (deterministic, not the doc's fixed 92%).
    stockout_pct = int(min(95, max(35, round(40 + scores.demand_volatility * 9 + unit * 12))))
    peak_share_pct = int(min(85, max(45, round(48 + scores.demand_volatility * 5 + unit * 10))))
