...
| `POST` | `/api/preflight/detect-roles` | 업로드 테이블 헤더로 정규 프레임 역할 추정(T57). 멀티시트 xlsx는 시트별 분리(T58). `files` 멀티파트 다건 |
| `POST` | `/api/preflight` | 선택 폼: `promotion_sheet`/`product_sheet`/`inventory_sheet`, `role_mappings`(JSON, T59) |
